...
# Modo de utilização
	Criar uma instância da classe GA_API_Handler fornecendo via
	parâmetro o diretório contendo o arquivo "client_secret" da
	Service Account. Um token de acesso temporário será disponibilizado
	para efetuar operações na API do Google Analytics.
...
#TODO: Implementar design pattern Singleton para esta classe
class GA_API_Handler:

	def __init__(self, dir_secrets='service_account_secrets', auth_method='service_account'):
		self.api = 'analytics'
		self.api_version = 'v3'
	...
	def _find_secrets(self, dir_secrets):
		...
		if len(secrets) < 1:
			raise FileNotFoundError('Nenhum arquivo .json encontrado no diretório "%s"' % dir_secrets)

		return secrets

	def _service_account_credentials(self):
		#TODO: Carregar credenciais para múltiplos 'client secrets'
		return ServiceAccountCredentials.from_json_keyfile_name(self.secrets[0], scopes=self.scopes)

	def _build_service_token(self):
		try:
			return build(self.api, self.api_version, credentials=self.credentials)

		except HttpError:
			self._alert_http_error(HttpError)

	def _alert_http_error(self, error):
		print('Ocorreu um erro durante a tentativa de acessar a API: %s %s'\
	...
	def _request_account_summary(self):
		try:
			response = self.service.management().accountSummaries().list().execute()
	...
	def _request_user_links(self, account_id):
		try:
			response = self.service.management().accountUserLinks().list(accountId=account_id).execute()
			self._log_api_request(response)
			return(response)
		except HttpError:
			self._alert_http_error(HttpError)
	...
	def query_users_by_account(self, account_id):
		...
		for item in request['items']:
			user = usr.User(user_id=item['userRef']['id'],
				user_account_id=item['entity']['accountRef']['id'],
				user_email=item['userRef']['email'],
				user_local_permission=item['permissions']['local'],
				user_global_permission=item['permissions']['effective'])
			users.append(user)
