...
class MySQL_DB:
    ...
    def _query_account_id(self, id_analytics):
        query = 'SELECT id FROM Account WHERE id_analytics = "%s"'\
            % id_analytics
        return self._execute_query(query, fetch=True)

    def _query_property_id(self, id_analytics):
        query = 'SELECT id FROM Property WHERE id_analytics = "%s"'\
            % id_analytics
        return self._execute_query(query, fetch=True)

    def _insert_property(self, Property, id_account):
        fk_id_account = self._query_account_id(id_account)[0][0]
        query = 'INSERT INTO Property(id_analytics, id_account,\
                 txt_property_name) VALUES ("%s", "%s", "%s")' % (
    ...
    def _insert_view(self, View, id_property):
        ...
        query = 'INSERT INTO View(id_analytics, id_property, txt_view_name)\
                 VALUES ("%s", "%s", "%s")' % (
            View.get_view_id(),
            fk_id_property,
            View.get_view_name())
    ...
    def _execute_query(self, query, fetch=False):
        ...
        try:
            ...
            if error.errno == 1062:
                print('Tentativa de inserir um registro duplicado: "%s¨'
                      % query)
                self.db_connection.rollback()
                pass
        finally:
    ...
    def insert_account(self, Account):
        query = 'INSERT INTO Account(id_analytics, txt_account_name) VALUES\
             ("%s", "%s")' % (
    ...
    def insert_user(self, User):
        ...
        query = 'INSERT INTO User (id_analytics, id_account, txt_user_email,\
             txt_user_local_permission, txt_user_global_permission) VALUES\
             ("%s", "%s", "%s", "%s", "%s")' % (
            User.get_user_id(),
            id_account,
            User.get_user_email(),
